...
/**
 * Default observer (hex pubkey) when `--observer` is omitted, from the
 * `DEFAULT_OBSERVER` env var. Unset means no default: text results still come
 * back, but every trust score is 0 (no trust perspective applied).
 */
internal val DEFAULT_OBSERVER = System.getenv("DEFAULT_OBSERVER").orEmpty()
